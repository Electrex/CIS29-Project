...
InputManager::InputManager(Player& player, sf::Sprite& sprite, sf::View& view, sf::RenderWindow& window) : mPlayer(player), mSprite(sprite), mView(view), mWindow(window), mAngle(0)
{
    kbmActive = true;
...
void InputManager::keyboard(const sf::Event& event)
{
    ...
    if (movement.x != 0.f && movement.y != 0.f)
        movement *= 0.707f;
    mPlayer.move((movement * mDeltaTime.asSeconds() * 500.f).x, (movement * mDeltaTime.asSeconds() * 500.f).y);
